$(document).ready(function() {
  ...
  $$('#municipalities-map').pinching(function(e) {
    noDraging = true;

    // console.log(e, e.touch.delta);
    if (e.touch.delta > 0) {
      zoomInn(1.2, 1);  
    } else {
      zoomOut(1.2, 1);
  ...
  window.scaleMap = function(scale, animation) {
    offset = ((4000-4000/scale)/2);
    viewSize = (4000 / scale);
  ...
  var zoomInn = function(multiplier, animation) {
    var viewBox = mapSVG.attr('viewBox');
    var oldScale = (4000/viewBox.width);
    scale = ((+oldScale)*(multiplier || 1.3));
    if(scale >=25) {
      return;
    }
    scaleMap(scale, (animation || 200 ));
    $('#zoom').val(scale)
  }

  var zoomOut = function(multiplier, animation) {
    var viewBox = mapSVG.attr('viewBox');
    var oldScale = (4000/viewBox.width);
    scale = ((+oldScale)/(multiplier || 1.3));
    if(scale <=0.9) {
